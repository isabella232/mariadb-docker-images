...
if ! testOutput="$(docker run --rm -e "MYSQL_ROOT_PASSWORD=$MYSQL_ROOT_PASSWORD" "$image" "$get_mysqld_pid")"; then
	echo >&2 'Mysqld not running. $get_mysqld_pid returned $testOutput'
	exit
fi
echo "successfully ran: $testOutput"
